...
window.projectStore = {
    ...
    // Auto-guardado
    autoSave() {
        if (this.isProjectInitialized() && window.storageService) {
            // Debounce para no guardar en cada tecla
            if (this._autoSaveTimeout) {
                clearTimeout(this._autoSaveTimeout);
            }
            this._autoSaveTimeout = setTimeout(() => {
                window.storageService.autoSave(this);
            }, 1000);
        }
    },
    ...
    loadProjectFromStorage(projectId) {
        ...
        const projectData = window.storageService.loadProject(projectId);
        if (projectData) {
    ...
    saveProject() {
        ...
        return window.storageService.saveProject(this.exportProject());
    },
    ...
    // Iniciar con proyecto guardado
    init() {
        if (!window.storageService) {
            console.log('⚠️ Storage service not loaded yet');
            return;
        }

        // Intentar cargar el proyecto actual
        const projectData = window.storageService.init();
        if (projectData) {
            Object.assign(this, projectData);
            console.log(`✅ Loaded last project: ${this.projectInfo.title}`);
        }
    }
};
