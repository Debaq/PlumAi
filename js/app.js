...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('versionControlView', () => ({
        ...
        // Crear datos genéricos simples para modo "Solo mirar"
        async createSimpleDemoData() {
            try {
                const project = Alpine.store('project');

                // IDs para los personajes
                const char1Id = window.uuid.generateUUID();
                const char2Id = window.uuid.generateUUID();

                // Dos personajes genéricos simples
                project.characters.push({
                    id: char1Id,
                    name: 'Personaje Principal',
                    role: 'protagonist',
                    description: 'El/la protagonista de la historia',
                    personality: 'Valiente y determinado/a',
                    background: 'Un pasado misterioso que se revelará a lo largo de la historia',
                    relationships: [],
                    notes: '',
                    avatar: null,
                    vitalStatusHistory: [
                ...
                project.characters.push({
                    id: char2Id,
                    name: 'Personaje Secundario',
                    role: 'supporting',
                    description: 'Compañero/a del protagonista',
                    personality: 'Leal y sabio/a',
                    background: 'Un aliado confiable en la aventura',
                    relationships: [],
                    notes: '',
                    avatar: null,
                    vitalStatusHistory: [
                ...
                // Dos capítulos con contenido genérico
                project.chapters.push({
                    id: window.uuid.generateUUID(),
                    title: 'Capítulo 1: El Comienzo',
                    number: 1,
                    summary: 'El inicio de nuestra historia',
                ...
                project.chapters.push({
                    id: window.uuid.generateUUID(),
                    title: 'Capítulo 2: El Desarrollo',
                    number: 2,
                    summary: 'La trama se desarrolla',
                ...
                // Crear commit inicial con los datos genéricos
                const projectData = project.exportProject();
                const author = {
                    name: localStorage.getItem('userName') || 'Usuario Demo',
                    email: localStorage.getItem('userEmail') || 'demo@pluma.local'
                };

                window.versionControl.commit(
                    projectData,
                    'Proyecto de demostración - Solo mirar',
                    author
                );
                ...
                // Recargar el estado del último commit
                await this.loadLastCommitState();
            } catch (error) {
                console.error('Error creando datos genéricos:', error);
            }
        },
