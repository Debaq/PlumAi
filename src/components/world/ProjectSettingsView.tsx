    ...
    <div className="flex flex-col h-full bg-background animate-in fade-in duration-300">
      ...
      <header className="h-16 border-b border-border flex items-center justify-between px-6 bg-card/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-lg font-black tracking-tight uppercase">{t('projectSettings.title')}</h2>
            ...
            <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-300">
              <section className="space-y-6">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <h3 className="text-xl font-black tracking-tight">{t('projectSettings.basics')}</h3>
                </div>
              ...
              <section className="p-6 rounded-3xl bg-primary/5 border border-primary/10 flex items-center justify-between">
                <div className="space-y-1">
                  <h4 className="font-bold flex items-center gap-2">
                    ...
                    {t('projectSettings.worldbuilder.active')}
                  </h4>
                  <p className="text-xs text-muted-foreground">{t('projectSettings.worldbuilder.description')}</p>
                </div>
                <Badge className="bg-primary px-4 py-1 rounded-full font-black">{t('projectSettings.worldbuilder.enabled')}</Badge>
              </section>
            </div>
          )}
          ...
          {activeTab === 'identity' && (
            <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-300">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-xl font-black tracking-tight">{t('projectSettings.visual.title')}</h3>
              </div>

              <section className="space-y-4">
                <Label className="text-[10px] font-black uppercase tracking-widest ml-1">{t('projectSettings.visual.banners')}</Label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="aspect-[3/1] rounded-2xl border-2 border-dashed border-border bg-muted/20 flex flex-col items-center justify-center gap-2 group cursor-pointer hover:bg-muted/30 transition-all">
                    ...
                    <span className="text-[10px] font-bold uppercase">{t('projectSettings.visual.bannerSidebar')}</span>
                  </div>
                  <div className="aspect-[3/1] rounded-2xl border-2 border-dashed border-border bg-muted/20 flex flex-col items-center justify-center gap-2 group cursor-pointer hover:bg-muted/30 transition-all">
                    <ImageIcon size={24} className="text-muted-foreground group-hover:scale-110 transition-transform" />
                    <span className="text-[10px] font-bold uppercase">{t('projectSettings.visual.bannerChapters')}</span>
                  </div>
                </div>
              </section>
          ...
          {activeTab === 'danger' && (
            <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-300">
              <div className="flex items-center gap-3 mb-2">
                ...
                <h3 className="text-xl font-black tracking-tight text-destructive">{t('projectSettings.danger.title')}</h3>
              </div>

              <section className="p-6 rounded-3xl border-2 border-destructive/20 bg-destructive/5 space-y-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-1">
                    <h4 className="font-bold text-sm">{t('projectSettings.danger.delete')}</h4>
                    <p className="text-xs text-muted-foreground">{t('projectSettings.danger.deleteWarning')}</p>
                  </div>
                  <Button variant="destructive" className="rounded-xl h-10 px-6 font-bold gap-2 shadow-lg shadow-destructive/20">
                    <Trash2 size={16} />
                    {t('projectSettings.danger.deletePermanent')}
                  </Button>
                </div>
